// Global variables - Make sure they're accessible globally
window.weeklyData = JSON.parse(localStorage.getItem("weeklyData") || "[]");
let weeklyData = window.weeklyData; // Keep local reference for compatibility
...
window.settings = settings; // Make settings globally accessible too
...
function debugNavigation() {
  ...
  console.log(`Found ${navItems.length} nav items and ${pages.length} pages`);
  
  navItems.forEach((item, index) => {
    const dataPage = item.getAttribute("data-page");
    const hasOnclick = item.hasAttribute("onclick");
    const hasHandler = !!item._navHandler;
    
    console.log(`Nav ${index}: "${item.textContent.trim()}" - data-page: ${dataPage}, onclick: ${hasOnclick}, handler: ${hasHandler}`);
  });
  
  pages.forEach((page, index) => {
    const isActive = page.classList.contains("active");
    console.log(`Page ${index}: "${page.id}" - active: ${isActive}`);
  });
...
function initializeNavigation() {
  ...
  navItems.forEach((item, index) => {
    ...
    if (item.hasAttribute("onclick")) {
      console.log(`Skipping external nav item ${index}:`, item.textContent.trim());
      return;
    }

    const targetPage = item.getAttribute("data-page");
    if (!targetPage) {
      console.log(`Skipping nav item ${index} without data-page:`, item.textContent.trim());
      return;
    }

    console.log(`Setting up nav item ${index}:`, targetPage);
    ...
    const navHandler = function (e) {
      try {
        ...
        alert("Navigation error occurred. Please check the console for details.");
      }
    };
...
function handleCalendarDate() {
  ...
  if (newShiftDate) {
    ...
    if (workDateInput) {
      ...
      console.log("Set calendar date:", newShiftDate);
    }
...
function handleEditShiftData() {
  ...
  if (editShiftData) {
    const shiftData = JSON.parse(editShiftData);
    ...
    console.log("Loaded edit shift data:", shiftData);
...
function calculatePay() {
  console.log("Starting pay calculation...");
  ...
  if (!shiftType) {
    if (typeof formValidator !== 'undefined') {
      formValidator.setFieldError('shiftType', 'Please select a shift type');
    }
    hasErrors = true;
  }
  ...
  if (!startTime) {
    if (typeof formValidator !== 'undefined') {
      formValidator.setFieldError('startTime', 'Please enter start time');
    }
    hasErrors = true;
  }
  ...
  if (!endTime) {
    if (typeof formValidator !== 'undefined') {
      formValidator.setFieldError('endTime', 'Please enter end time');
    }
    hasErrors = true;
  }
  ...
  if (!workDate) {
    if (typeof formValidator !== 'undefined') {
      formValidator.setFieldError('workDate', 'Please select work date');
    }
    hasErrors = true;
  }
...
function addToWeekly() {
  ...
  window.weeklyData = weeklyData; // Update global reference
